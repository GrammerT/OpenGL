...
{
    vec3 s = normalize(vec3(Light.Position - position));//! 模型点到灯的向量
    vec3 v = normalize(-position.xyz);  //! 对上面的向量取反方向 (灯射向模型点的向量)
    ...
    vec3 spec = vec3(0.0);
    if( sDotN > 0.0 )
        spec = Light.LightSpecular * Material.RateSpecular * pow( max( dot(r,v), 0.0 ), Material.Shininess );//! 计算镜面光强
